...
// The legacy static interface has no login flow. These integrations are
// intentionally public so its existing controls continue to work.
app.use('/api/weather', weatherRoutes);
app.use('/api/spotify', spotifyRoutes);
app.use('/api/system', requireAuth, systemRoutes);
app.use('/api/vision', requireAuth, visionRoutes);

// Legacy frontend compatibility: its WhatsApp button uses this endpoint.
app.post('/api/launch/whatsapp', (req, res, next) => {
  req.body = { ...(req.body || {}), appName: 'whatsapp' };
  return launchSystemApp(req, res, next);
});

// Direct Spotify compatibility Callback Route
