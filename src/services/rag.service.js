...
const getPineconeHost = async () => {
  ...
  try {
    ...
    logger.warn(`RAG host resolution failed: ${error.message}`);
    return null;
  }
};
...
export const querySemanticMemory = async (text, userId = null, limit = 4) => {
  ...
  try {
    ...
    logger.warn(`Pinecone query failed: ${error.message}`);
    return [];
  }
};
...
export const buildRAGContext = async (queryText, userId = null) => {
  try {
    ...
    logger.warn(`Failed to construct RAG context: ${error.message}`);
    return '';
  }
};
